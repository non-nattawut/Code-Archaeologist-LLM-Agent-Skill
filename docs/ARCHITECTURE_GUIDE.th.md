...
archaeologist.py  project | flow | both | check | report | brief   (Entrypoint หลัก)
  ...
  flow     -> build_flow ---------------------------------+--> render_explorer()
       ทั้งคู่ประมวลผลผ่าน: py_extract.py     (Python,        tree-sitter)
          ...
          ตัวช่วยสกัด AST: call_ctx.py       (CFG call sites: line, loop, cond, arms)
                          doc_text.py       (ล้างและจัดรูปแบบ docstring สากล)
...
### 3.0 ตาราง Checklist รายไฟล์สำหรับการศึกษาและตรวจสอบสถาปัตยกรรม (Reading Checklist)

ใช้ Checklist ตารางนี้เพื่อติดตามความคืบหน้าในการศึกษาและตรวจสอบโค้ดครบทั้ง 32 ไฟล์:
